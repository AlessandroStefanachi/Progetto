...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="/Progetto/Smarty/css/utenti.css">

    <script src="/Progetto/Smarty/js/utenti.js"></script>
...
<div class="d-flex " style="display: flex ; flex-wrap: wrap; width: 100%;flex-grow: 1;">
    <div class="d-flex flex-row justify-content-center pt-5" style="width: 100%">
        <div class="jumbotron utenti" style="max-height: 40vw">
       <ul>
           {foreach from=$utenti item=$a }
         <li><div class="d-flex flex-row justify-content-center pt-3" style="width: 100%">
                 <span class="mr-auto">{$a->getUsername()}</span>
                 <span class="mx-auto">Status: {$a->getRuolo()}</span>
                 {if $a->getRuolo()=='banned'}<a class="ml-auto btn btn-sm" href="/Progetto/Admin/unban?id={$a->getUsername()}">unban</a>{else}<a class="ml-auto btn btn-sm"  href="/Progetto/Admin/ban?id={$a->getUsername()}">ban</a>{/if}
                 <a class="ml-auto btn btn-sm"  href="/Progetto/Admin/eliminaUtente?id={$a->getUsername()}">elimina</a>
             </div></li>
           {/foreach}
       </ul>
        </div>
