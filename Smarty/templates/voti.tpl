...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="/Progetto/Smarty/css/voti.css">

    <script src="/Progetto/Smarty/js/voti.js"></script>
...
<div class="d-flex " style="display: flex ; flex-wrap: wrap; width: 100%;flex-grow: 1;">
    <div class="d-flex flex-row justify-content-center pt-5" style="width: 100%">
        <div class="jumbotron voti" style="max-height: 40vw">
            <ul>
                <li>
                    <div class="d-flex flex-row justify-content-center pt-2" style="width: 100%">
                        <span class="mr-auto">VOTO</span> <span class="mx-auto">ID_EPISODIO</span><span class="ml-auto">AUTORE</span><span class="ml-auto pl-5"></span><span class="ml-auto pl-2"></span></div></li>
                {foreach from=$voti item=$a }
                    <li>
                        <div class="d-flex flex-row justify-content-center pt-2" style="width: 100%">
                            <span class="mr-auto">{$a->getVoto()}</span> <a class="mx-auto" href="/Progetto/Episodio/info?id={$a->getIdEpisodio()}">{$a->getIdEpisodio()}</a><span class="ml-auto">{$a->getAutore()}</span>
                            <form class="ml-auto" method="post" action="/Progetto/Admin/cancellaVoto">
                                <input type="hidden"name="autore" value="{$a->getAutore()}">
                                <input type="hidden"name="id" value="{$a->getIdEpisodio()}">
                                <input class="btn btn-sm"type="submit" id="creabutton" value="elimina" >
                            </form>
                        </div></li>
                {/foreach}
            </ul>
